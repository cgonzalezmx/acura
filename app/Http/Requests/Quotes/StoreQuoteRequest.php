...
class StoreQuoteRequest extends FormRequest
{
    ...
    public function messages(): array
    {
        return [
            ...
            'entries.*.title' => 'Falta el nombre del punto de muestreo de la partida :position',
            'entries.*.form_factor' => 1,
            'entries.*.objective' => 1,
            'entries.*.result_time_lapse' => 1,
            'entries.*.sample_type' => 1,
            'entries.*.refrigerated' => 1,
            'entries.*.quantity' => 1,
            'entries.*.reports.*.selected_keys' => 1,
            'entries.*.reports.*.observation' => 1,
            'entries.*.reports.*.thresholds' => 1,
            'entries.*.expenses.*.cost' => 1,
            'entries.*.included_parameters' => 1,
            'entries.*.included_parameters.*.quantity' => 1,
